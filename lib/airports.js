...
const state = {
  ...
  navIls: null,          // "ICAO|RWY" -> same shape, from data/navdata-ils.json (Navigraph via XP Custom Data)
  navdata: null,         // {cycle, validTo, source, count, extractedAt} when the file is present
  loadedAt: null,
  refreshing: false,
  lastError: null,
};
...
function indexAll(ilsCsvPath) {
  ...
  for (const r of parseCsv(read('airport-frequencies.csv'))) {
    ...
    if (!Number.isFinite(mhz) || mhz < 108 || mhz > 137) continue; // VHF only
    // 108–117.95 is the NAV band. Broadcast types legitimately live there
    // (Australia's AERIS transmits ATIS over VORs — tune the NAV radio);
    // a comm position "in" the nav band is data corruption, drop it.
    const navBand = mhz < 118;
    if (navBand && !['ATIS', 'AWOS', 'ASOS'].includes(t)) continue;
    if (!freqs.has(r.airport_ident)) freqs.set(r.airport_ident, []);
    freqs.get(r.airport_ident).push({ type: t, label: FREQ_LABEL[t], desc: r.description || '', mhz, navBand });
  }
  ...
  // Navigraph-derived global ILS (produced by tools/extract-navdata.js).
  // Optional: absent file just means NASR-only, exactly as before.
  //
  // LICENSING GUARD: a Navigraph subscription covers the subscriber, not an
  // audience. In PUBLIC_MODE the file is ignored even if present, so a hosted
  // instance can never redistribute it — enforced here rather than relying on
  // the operator not copying data/ to the server.
  const navIls = new Map();
  let navdata = null;
  const navFile = path.join(DATA_DIR, 'navdata-ils.json');
  state.navSkipped = false;
  if (PUBLIC_MODE) {
    state.navSkipped = fs.existsSync(navFile); // present but deliberately unused
  } else {
    try {
      const doc = JSON.parse(fs.readFileSync(navFile, 'utf8'));
      for (const [key, v] of Object.entries(doc.ils || {})) {
        navIls.set(key, { locFreq: v.f, course: v.c, category: v.cat || null, locId: v.id || null, type: v.cat || null });
      }
      navdata = doc.meta || null;
    } catch (e) { /* no navdata file — fine */ }
  }
  ...
  state.navIls = navIls; state.navdata = navdata;
...
function airportInfo(icao) {
  ...
  // ILS per runway end: Navigraph (AIRAC-current, global) wins; FAA NASR fills gaps.
  const ilsFor = rwy =>
    (state.navIls && (state.navIls.get(`${icao}|${rwy}`) || state.navIls.get(`${ident}|${rwy}`)))
    || state.ils.get(`${fid}|${rwy}`) || null;
  const rwys = (state.runways.get(ident) || []).map(r => {
    const ends = [
      { ident: r.leIdent, headingT: r.leHeadingT, displacedFt: r.leDisplacedFt, ils: ilsFor(r.leIdent) },
      { ident: r.heIdent, headingT: r.heHeadingT, displacedFt: r.heDisplacedFt, ils: ilsFor(r.heIdent) },
    ];
    return { ...r, ends };
  });
  return {
...
function ilsSource() {
  if (state.navSkipped) {
    return `FAA NASR ${state.ilsCycle || '—'} (US only) — Navigraph data present but not served in public mode`;
  }
  if (state.navdata && state.navIls && state.navIls.size) {
    const expired = state.navdata.validTo && Date.now() > Date.parse(rewriteNavDate(state.navdata.validTo)) + 24 * 3600 * 1000;
    return `Navigraph ${state.navdata.cycle}${expired ? ' (CYCLE EXPIRED — rerun update-navdata)' : ''} + NASR fallback`;
  }
  return state.ilsCycle ? `FAA NASR ${state.ilsCycle} (US only)` : 'none';
}

// "03/SEP/2026" -> "03 Sep 2026" so Date.parse understands it
function rewriteNavDate(s) {
  return String(s).replace(/\//g, ' ').replace(/([A-Z])([A-Z]+)/, (m, a, b) => a + b.toLowerCase());
}
...
function status() {
  return {
    ...
    navIlsCount: state.navIls ? state.navIls.size : 0,
    ilsSource: ilsSource(),
