...
// Mirror what SI's own client shows. Derived empirically (2026-08-30) by
// diffing the client's airport lookup against this API for KOAK/KSFO/KLAX/
// KDFW/YSSY/EGLL/KSNA/KSAN: the client hides the SECOND entry of each type in
// the API's array order, but only when that entry carries a callsign. 29/29
// hidden frequencies fit, and both counter-cases confirm it (EGLL GND and
// YSSY TWR have callsign:null in that slot and are shown in full).
//
// This matters operationally: KOAK TWR 127.2 (real-world South Field) is one
// of the hidden ones, which is consistent with calls on it going unanswered.
// Array order is authoritative here — never sort before filtering.
function siVisibleComms(comms) {
  ...
  for (const c of Array.isArray(comms) ? comms : []) {
    ...
    const idx = perType.get(t) || 0;
    perType.set(t, idx + 1);
    if (idx === 1 && c.callsign) continue; // hidden by SI's client
    out.push({ type: t, mhz, callsign: c.callsign || null });
  }
  return out;
}
...
function collapseComms(comms) {
  ...
  for (const c of siVisibleComms(comms)) {
    if (!SI_TYPE_LABEL[c.type]) continue; // drops RMP/AIR and anything unmodelled
    if (!byType.has(c.type)) {
      byType.set(c.type, { type: c.type, label: SI_TYPE_LABEL[c.type], callsign: c.callsign, freqs: [] });
    }
    const e = byType.get(c.type);
    if (!e.freqs.includes(c.mhz)) e.freqs.push(c.mhz); // dedupe (KSNA lists CLR 118.0 twice)
    if (!e.callsign && c.callsign) e.callsign = c.callsign;
  }
  return [...byType.values()]
    .map(e => {
      // Prefer a COM-band frequency as the printed one: several fields
      // broadcast ATIS over VORs (KJFK 115.4/117.7, KSFO 113.7, YSSY 115.55)
      // which a COM radio cannot tune. SI's own compact panel does the same —
      // it shows KJFK ATIS 128.725, not 115.4. Nav-band entries are kept but
      // demoted and flagged so the sheet can say "tune NAV radio".
      const comm = e.freqs.filter(f => f >= 118);
      const nav = e.freqs.filter(f => f < 118);
      const ordered = comm.length ? [...comm, ...nav] : e.freqs;
      return {
        type: e.type, label: e.label, callsign: e.callsign,
        mhz: ordered[0], navBand: ordered[0] < 118,
        altCount: ordered.length - 1, all: ordered,
      };
    })
