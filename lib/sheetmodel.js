...
async function buildSheet(user, { diskCache = true, network = 'auto', siAuth = null } = {}) {
  ...
  let depNet = null, arrNet = null, vatsim = null;
  ...
  if (net.resolved === 'sayintentions') {
    ...
    if (key) {
      const [siDep, siArr] = await Promise.all([si.fetchSiWx(depIcao, key), si.fetchSiWx(arrIcao, key)]);
  ...
  return {
    ...
    vatsim,
    ofp,
    dep: { info: depInfo, wx: depWx, netAtis: depNet, likelyRwys: depLikely },
    arr: { info: arrInfo, wx: arrWx, netAtis: arrNet, likelyRwys: arrLikely },
