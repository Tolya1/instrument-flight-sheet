...
async function buildSheet(userid) {
  const status = { simbrief: 'live', wx: 'ok', peAtis: 'ok', airportData: 'ok' };

  let rawOfp;
  try {
    rawOfp = await fetchOfp(userid);
  } catch (e) {
    rawOfp = readCachedOfp();
    status.simbrief = rawOfp ? `cached (${e.message})` : `failed (${e.message})`;
    if (!rawOfp) return { error: `SimBrief unreachable and no cached OFP: ${e.message}`, status };
  }
