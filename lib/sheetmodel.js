...
// Which conventions an airport follows: inHg + feet + FAR 91.185 in the US and
// its territories, hPa + metres + ICAO/SERA everywhere else. Prefixes: K =
// CONUS, PA/PH/PG/PM/PO/PW = Alaska/Hawaii/Pacific, TI/TJ = USVI/Puerto Rico.
function isUsIcao(icao) {
  return typeof icao === 'string' && /^(K|PA|PH|PG|PM|PO|PW|TI|TJ)/.test(icao.toUpperCase());
}
...
function wxBlock(icao, awcMetar, sbMetarRaw, sbTaf, awcTaf) {
  ...
  // No altimeter group parsed (or no METAR at all): fall back to the region's
  // convention rather than defaulting everyone to inches.
  if (!parsed.altimUnit) parsed.altimUnit = isUsIcao(icao) ? 'inHg' : 'hPa';
...
module.exports = { buildSheet, windComponents, parseRunwayList, wxBlock, localTime, resolveNetwork, NETWORKS, isUsIcao };
