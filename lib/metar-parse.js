...
function parseMetar(raw) {
  const out = {
    ...
    altimInHg: null, altimHpa: null, altimUnit: null, // 'inHg' (A-group) or 'hPa' (Q-group)
  ...
  try {
    ...
    for (; i < toks.length; i++) {
      ...
      if ((m = t.match(/^(\d{2})(\d{2})(\d{2})Z$/)) && !out.timeZ) {
        ...
        // Observations are never future-stamped beyond clock skew: anything
        // more than an hour ahead belongs to the previous month (e.g. a
        // "302355Z" seen on the 30th at 08:00Z).
        if (d.getTime() - now.getTime() > 3600 * 1000) {
      ...
      if ((m = t.match(/^A(\d{4})$/))) {
        ...
        out.altimUnit = 'inHg'; // the station reports inches — set that on the subscale
        continue;
      }
      if ((m = t.match(/^Q(\d{4})$/))) {
        out.altimHpa = +m[1];
