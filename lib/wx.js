...
// Short per-request-set cache so a public deployment doesn't hammer AWC.

const WX_TTL_MS = 3 * 60 * 1000;
const WX_CACHE_MAX = 500;
const wxCache = new Map(); // `${kind}:${ids}` -> {at, data}

function cacheGet(key) {
  const hit = wxCache.get(key);
  return hit && Date.now() - hit.at < WX_TTL_MS ? hit.data : null;
}
function cachePut(key, data) {
  wxCache.set(key, { at: Date.now(), data });
  if (wxCache.size > WX_CACHE_MAX) {
    const oldest = [...wxCache.entries()].sort((a, b) => a[1].at - b[1].at)[0];
    if (oldest) wxCache.delete(oldest[0]);
  }
}
...
async function awcJson(kind, ids) {
  ...
  const url = `https://aviationweather.gov/api/data/${kind}?ids=${ids.map(encodeURIComponent).join(',')}&format=json`;
